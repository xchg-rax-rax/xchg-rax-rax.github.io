...
The Rensenware malware is a piece of ransomware with a twist.<sup>[1]</sup>
Rather than the usual demands levelled against victims, i.e. for some amount of bitcoin or ethereum to be transferred into an anonymous wallet, Rensenware demands something different from its victims.
...
For people not familiar with the bullet hell genre this may be a far more challenging prospect than coming up with a few meagre million in bitcoin!
So, to help those lost souls, at the end of our analysis we will write a short program that can trick Rensenware into thinking that you are indeed a master of Touhou games and into decrypting your files for you.

Let's begin.

## Behavioural Analysis

To get an understanding of the way in which Rensenware operates and the kinds of behaviour we should expect to see when we begin more detailed technical analysis, we will start out by performing some basic behavioural analysis of the malware in a virtual machine.
...
This isn't a sophisticated approach and we could go further, seeking to record all changes made to the file system, registry and so on.
...
Even though Defender is completely disabled SmartScreen still flags the file as malicious.
...
A few seconds pass and suddenly the icon of our text file containing precious haiku turns blank and its name acquires the `.RENSENWARE` extension.
A few seconds more and a red warning message containing a cute anime *waifu* appears on our screen informing us that all our files have been encrypted and that to get them back we must score 200 million in Touhou Seirensen: Undefined Fantastic Object on "Lunatic" difficulty!
...
The cute anime girl's words were not an empty threat it seems!
...
Starting a new game in lunatic mode as the malware requests we see that the message changes to "Process Working" and the malware starts to keep track of our score.
If we can now just reach 200 million points we will be able to rescue our files!

![Playing Touhou](/images/dynamic7.png)

Well as it turns out Touhou on lunatic mode is actually ridiculously difficult and we weren't even able to score 1 million, never mind the 0.2 billion the malware demands.

![Touhou Playing](/images/dynamic8.png)

Since we stand no chance of winning against the malware on its own terms we must understand how it operates by reverse engineering its binary and then use this knowledge to find a way to save our precious files.
...
In this case it tells us that the Rensenware binary is, unsurprisingly, a Windows PE file.
More interestingly however it also tells us that it is in fact a .Net assembly.
.Net assemblies that have not been obfuscated can be quite reliably converted back into C# source code and so the fact that this binary is a .Net assembly means that reversing it will likely be quite easy.
...
The first thing to note is that the SHA256 sum of the binary is `7bf5623f0a10dfa148a35bebd899b7758612f1693d2a9910f716cf15a921a76a`, which is consistent with the sum reported in the Bleeping Computer article which first broke the news of the malware<sup>[2]</sup>.

![General Metadata](/images/pebear1.png)

Next by inspecting the COFF File header data we see that the TimeDateStamp, a hard-coded value in the header that indicates when the file was created<sup>[3]</sup>, tells us that the binary was compiled at 1:32pm UTC on the 6th of April 2017.
While this value is very easy to spoof, it is a reasonable date as the malware was first reported by Bleeping Computer on the same day (which is suspicious for other reasons, but I shan't speculate about that...).
...
This is particularly damning evidence in fact, as it gives us the full path of the directory in which the binary was compiled.
We see that it was compiled with Visual Studio 2017 and that the username of the author is *mkang*.
This is potentially a very valuable piece of information with respect to attribution and, if accurate, a significant oversight on the part of the Rensenware author.

![RSDSI Table](/images/pebear4.png)

With this discovery our basic static analysis is done and we move on to the more advanced stage where we seek to reverse engineer the binary by decompiling it to its source code and, by inspecting this source code, infer its functionality.
...
Firstly there is a `List<string>` named `encryptedFiles` which presumably is used by the program to keep track of the files it has already encrypted.
Following this are two strings `KeyFilePath` and `IVFilePath`.
The names of these variables imply that Rensenware likely uses the AES block cipher to encrypt the user's files as this is the most common algorithm for file encryption that also employs a key and a separate IV, an initialization vector.
Furthermore, given that paths are defined for these values it seems likely that they will at some point be saved to disk, possibly after the user has achieved the specified high score.
...
Next it allocates some memory for the Key and IV on the heap and randomly generates values for them.
...
The program then starts iterating over the logical drives.
First it checks if the current drive contains the System32 directory and if it does it obtains a list of all the subdirectories of the user's home directory.
For each of these directories it then obtains a list of all of the files contained in that directory and its subdirectories.
...
In the event that the drive does not contain System32 then the malware simply encrypts all files with any of the specified extensions, in much the same way as it did for the files contained in the user's home directory.
Following this the `frmWarning` class is run, presumably to show the ransom warning to the user and to verify if they have successfully overcome the challenge.

![Crypt Method](/images/source6.png)

The only other method in the `Program` class is the `Crypt` method.
This method is responsible for both encrypting and decrypting files, with its behaviour being determined by the optional argument `IsDecrypt`.
...
The file is read in and its bytes are either encrypted or decrypted depending on the `IsDecrypt` flag value.
...
Now that we've thoroughly analysed the `Program` class we can take a look at `frmWarning` to understand more about its behaviour.
...
The majority of the logic of the `frmWarning` class is contained in its constructor.
...
If a handle to the remote process has already been opened then a second flag `_flag_billion`, which indicates if the ransom condition has been met, is checked, and if it's true we break out of the loop.
...
This value is then compared with 3, and if it's not equal then a message is displayed on the GUI indicating that the game difficulty is not set to Lunatic mode, and after a sleep the loop continues.

![frmWarning Method 3](/images/sourceA.png)

If the check that lunatic mode is enabled passes then a message displaying the text "Process Working" is shown on the UI and a second check is then performed.
This second check reads a 4 byte integer from address 4918340 in the game's address space and compares its value with 20 million, and if it is greater it sets `_flag_billion`, otherwise it just resets the buffer used to read in the integer.
A sleep follows and then, if both the flags were set, the loop will be broken out of on the next iteration; otherwise the checks will continue until they are.
...
Next the list of encrypted files in the `Program` object is iterated over and `Program.Crypt` is called on each with the `IsDecrypt` flag set; a progress bar is updated after each file is decrypted.
Finally a message indicating that the decryption was successful is displayed to the user.

The final class `frmManualDecrypter` is simply responsible for implementing the manual decryption functionality that allows users who have passed the test and obtained the Key and IV values used to encrypt their files to manually decrypt any files that the automated decryption procedure in `frmWarning` failed to decrypt.
...
To create a decrypter we will simply create a binary which is given the friendly name 'th12' as required by `frmWarning` and writes correct values at the addresses the binary checks to verify the difficulty and the score.
...
Lo and behold it is indeed the same value as our score in the game (well, almost — the game multiplies this number by 10 when displaying it so that your score is a bigger number).

![Finding Score in Debugger](/images/debugger1.png)

If we just manually modify this 32-bit value in the debugger so that it exceeds 20 million the malware is tricked into believing that we have passed its test and it decrypts all of the files it encrypted.
Victory!
...
It's not quite as simple as this due to ASLR and the fact that the base address of Windows binaries is by default at or around 0x400000, and even if the binary is not mounted there then very often a locale file may be within this range.
If we try and do VirtualAlloc when something else is already using the pages we require then it will simply fail and (unless we are very lucky and the pages we need are allocated with write permissions) any attempt to write to these addresses will cause a segmentation fault.
...
Running the Rensenware executable to again encrypt our files and then running this program with its executable renamed to `th12.exe` we see that indeed the malware is again tricked into decrypting our files.

![Decrypter/Forcer](/images/decrypter.png)

And with that Rensenware is thoroughly defeated!
Beating Touhou Seirensen ~ Undefined Fantastic Object on Lunatic difficulty, however, remains a challenge, and may prove far more difficult.
